...
import kotlinx.android.synthetic.main.fragment_teacher_absent.view.*
...
class TeacherAbsent : Fragment(), TeacherAdapter.TeacherItemListener {
    ...
    override fun onCreateView(
        ...
        val view =  inflater.inflate(R.layout.fragment_teacher_absent, container, false)

        navController = Navigation.findNavController((requireActivity() as AppCompatActivity), R.id.nav_host_fragment)
        recyclerView = view.recyclerView_absent
        recyclerView.layoutManager = LinearLayoutManager(context)

        viewModel = ViewModelProvider(requireActivity()).get(HomeViewModel::class.java)
        viewModel.absentData.observe(viewLifecycleOwner, Observer {
    ...
    override fun onItemClick(item: TeacherItemList) {
        Log.i("teacher_abs","selected item:${item.name}")
